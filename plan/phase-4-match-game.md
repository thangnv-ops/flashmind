...
- [x] `initGame()`: tách cards thành term + definition, xáo trộn ngẫu nhiên
- [x] **Logic ghép thẻ hoàn chỉnh:**
  - Click thẻ 1 → `status: 'selected'`
  - Click thẻ 2 khớp pairId + khác type → `status: 'correct'` → biến mất (AnimatePresence)
  - Không khớp → `status: 'wrong'` → lắc (motion animate x) → reset về `idle` sau 500ms
- [x] **Timer:** millisecond precision, `setInterval(10ms)`
