...
/** Generate noise letters not in the term (4 by default). */
function generateNoiseLetters(term: string, count = 4): string[] {
  const termSet = new Set(term.toLowerCase().replace(/\s/g, '').split(''));
  const candidates = 'abcdefghijklmnopqrstuvwxyz'.split('').filter(c => !termSet.has(c));
  return fisherYates(candidates).slice(0, count);
}

/** Build a pool of letter tiles: term letters + noise letters, all shuffled. */
function buildPool(term: string): LetterTile[] {
  ...
  return fisherYates([...termLetters, ...noiseLetters]).map((l, i) => ({
    id: `tile-${i}-${l}`,
    letter: l,
  }));
}

/** Check built tiles against term (strip spaces, case-insensitive). */
function checkAnswer(built: LetterTile[], term: string): boolean {
  const builtStr = built.map(t => t.letter).join('').toLowerCase();
  return builtStr === term.replace(/\s/g, '').toLowerCase();
}
...
export const WordBuilder: React.FC = () => {
  ...
  // Build pool when card changes
  useEffect(() => {
    if (!currentCard) return;
    setPool(buildPool(currentCard.term));
    setBuilt([]);
  ...
  const handleCheck = useCallback(() => {
    ...
    const correct = checkAnswer(built, currentCard.term);
    if (correct) {
      setScore(s => s + 1);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-6">
        <div className="bg-white rounded-2xl border p-8 max-w-sm w-full text-center shadow-sm">
          ...
          <p className="text-slate-500 mb-6">Bạn đã xây xong {cards.length} từ</p>
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className={cn('rounded-2xl border-2 p-6 mb-6 transition-colors', feedbackColor)}>
          ...
          <button
            onClick={handleCheck}
            disabled={gameState !== 'playing' || built.length === 0}
